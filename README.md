...
<div id="hud">
    <div><span class="l">CRÉDITOS</span><span id="v-cash" class="v">$0</span></div>
...
<div id="btn-teleport" onclick="teleportToDinos()">🎯</div>

<div id="viewport">
    <div id="map" class="map-normal"><div id="entities-layer"></div></div>
</div>

<button id="btn-confirm" class="btn" style="position:fixed; bottom:110px; left:50%; transform:translateX(-50%); width:220px; display:none; z-index:4000; background:var(--incomum); border: 2px solid #fff" onclick="confirmPlacement()">SOLTAR AQUI ✅</button>

<div id="modal-shop" class="modal">
    <div class="m-card">
        <h2 style="text-align:center; color:var(--n)">LOJA DE EMBRIÕES</h2>
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px">
            <button class="btn" onclick="buy('carn', 30000)">🥩 CARNÍVOROS<br>$30k</button>
            <button class="btn" onclick="buy('herb', 20000)">🌿 HERBÍVOROS<br>$20k</button>
            <button class="btn" onclick="buy('fly', 45000)">🦅 VOADORES<br>$45k</button>
            <button class="btn" onclick="buy('wat', 70000)">🦈 AQUÁTICOS<br>$70k</button>
        </div>
        <button class="btn" style="background:var(--ceno); color:#000; margin-top:15px" onclick="buy('ceno', 1)">❄️ OVO CENOZOICO (1 DNA)</button>
...
<div id="modal-lab" class="modal">
    <div class="m-card">
        <h2 style="text-align:center; color:var(--hibrido)">LABORATÓRIO</h2>
...
<div id="modal-inv" class="modal"><div class="m-card"><h2>INVENTÁRIO</h2><div id="inv-list"></div><button class="btn" onclick="closeModals()">FECHAR</button></div></div>
<div id="modal-index" class="modal"><div class="m-card"><h2>DINOPÉDIA</h2><div id="idx-grid" style="display:grid; gap:8px"></div><button class="btn" onclick="closeModals()">FECHAR</button></div></div>

<div id="panel">
    <h2 id="p-name"></h2>
    <div id="p-stats"></div>
    <button class="btn" onclick="upgradeDino()">🥩 ALIMENTAR ($<span id="p-up-cost">0</span>)</button>
    <button class="btn" style="background:#f1c40f; color:#000" onclick="initMove()">🚚 MOVER</button>
...
<nav>
    <div class="nav-item" onclick="openModal('shop')"><span>🛒</span>LOJA</div>
    <div class="nav-item" onclick="openModal('lab')"><span>🧪</span>LAB</div>
    <div class="nav-item" onclick="openModal('inv')"><span>🧬</span>INV</div>
    <div class="nav-item" onclick="openIndex()"><span>📖</span>INDEX</div>
    <div class="nav-item" id="btn-food"><span>🥩</span>CARNE</div>
</nav>
...
let state = {
    cash: 250000, food: 100000, cenoDNA: 0,
...
function generate(cat, forced = null) {
    ...
    let icons = { carn: "🦖", herb: "🦕", fly: "🦅", wat: "🦈", ceno: "🦣", hibrido: "🧬" };
    if(!state.discovered.includes(name)) state.discovered.push(name);
    
    return { name, cat, rar, lvl: 1, x:0, y:0, img: icons[cat] || "🦖", world: (cat === 'ceno' ? 'ceno' : 'normal') };
}
...
function fuse(type) {
    ...
    if(idxA !== -1 && idxB !== -1) {
        ...
        state.inv.push(generate('hibrido', type)); alert("HÍBRIDO CRIADO!"); closeModals(); render();
    } else alert("Requisitos: " + reqA + " e " + reqB + " no Nível 30!");
}
...
function openIndex() { openModal('index'); const g = document.getElementById('idx-grid'); g.innerHTML = ''; state.discovered.sort().forEach(n => g.innerHTML += `<div style="padding:10px; background:rgba(255,255,255,0.1); border-radius:10px">🦴 ${n}</div>`); }
